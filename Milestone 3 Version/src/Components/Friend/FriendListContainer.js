...
class FriendListContainer extends React.Component {
    ...
    updateFriends = () => {
        ...
        firebaseDb.db.collection('friendlist').doc(uid).onSnapshot(docSnapshot => {
            ...
            Promise.all(promise).then(() => this.setState({allFriends: allFriends, friendsFiltered: allFriends, isLoading: false}))
        })
    }
    ...
    renderFriend = friend => {
        return (
            <TouchableOpacity onPress={() => {this.props.navigation.navigate("ViewProfileDelete",
